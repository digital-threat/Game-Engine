...
#include <../../engine/include/vk_helpers.h>
#include <../../engine/include/vk_pipelines.h>
#include <../../engine/vendor/glm/glm/gtc/quaternion.hpp>
...
void Engine::UpdateSceneDescriptorSet(VkDescriptorSet sceneSet, FrameData& currentFrame)
{
	...
	writer.WriteImage(3, mShadowmapTarget.imageView, TextureManager::Instance().GetSampler("NEAREST_MIPMAP_LINEAR"),
...
void Engine::RenderRaster(VkCommandBuffer cmd, FrameData& currentFrame)
{
	...
	VkRenderingAttachmentInfo colorAttachment =
			ColorAttachmentInfo(mColorTarget.imageView, &clear, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
	...
	for (auto& object : mApplication->mRenderContext.instances)
	{
		...
		vkCmdPushConstants(cmd, mMeshPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
						   sizeof(RasterPushConstants), &pushConstants);
